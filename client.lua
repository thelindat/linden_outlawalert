...
local currentStreetName, intersectStreetName, lastStreet, speedlimit = nil, nil, nil, 0
...
Citizen.CreateThread(function()
    ...
    while true do
        ...
        if currentStreetName ~= lastStreet and GetVehiclePedIsUsing(playerPed) then speedlimit = speedlimitValues[currentStreetName] end
        Citizen.Wait(500)
    end
end)
	...
	while true do
        ...
        if (not isPlayerWhitelisted or Config.Debug) then
            ...
            if GetVehiclePedIsUsing(playerPed) ~= 0 then
                ...
                if vehicle then
                    if Config.Timer['Shooting'] == 0 and not BlacklistedWeapon(playerPed) and IsPedShooting(playerPed) and not IsPedCurrentWeaponSilenced(playerPed) and IsPedArmed(playerPed, 4) then
                        sleep = 50
                    ...
                    if Config.Timer['Speeding'] == 0 then
