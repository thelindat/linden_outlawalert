...
local isPlayerWhitelisted, playerPed, playerCoords, job, rank, firstname, lastname, phone, isLoaded = false
-- ESX Framework Stuff ---------------------------------------------------------------
ESX = nil
...
Citizen.CreateThread(function()
    while not isLoaded do Citizen.Wait(0) end
...
Citizen.CreateThread(function()
    while true do
        Citizen.Wait(0)
        if not isLoaded and (not IsPedStill(PlayerPedId()) or GetVehiclePedIsUsing(PlayerPedId()) ~= 0) then
            ...
            isLoaded = true
        elseif isLoaded then return false
        end
    end
end)
