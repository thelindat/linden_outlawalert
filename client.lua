...
local currentStreetName, intersectStreetName, lastStreet, speedlimit = nil, nil, nil, nil
...
Citizen.CreateThread(function()
    ...
    while true do
        ...
        if (currentStreetName ~= lastStreet and GetVehiclePedIsUsing(playerPed)) or not speedlimit then speedlimit = speedlimitValues[currentStreetName] end
        Citizen.Wait(500)
    end
end)
...
function zoneChance(type, playerCoords, street, zoneMod)
    local zone, zoneid, sendit = GetLabelText(GetNameOfZone(playerCoords.x, playerCoords.y, playerCoords.z)), GetZoneAtCoords(playerCoords.x, playerCoords.y, playerCoords.z), false
        ...
        local scumminess = {[0] = 0.75, [1] = 5, [2] = 4, [3] = 3, [4] = 2, [5] = 1}
        local zoneScum = scumminess[GetZoneScumminess(zoneid)]
        zoneMod = zoneMod * zoneScum
...
function vehicleData(vehicle)
    ...
    if vehicleClass then
        ...
        if vehicleClass == 8 then vehicleDoors = ''
        ...
        if (not isPlayerWhitelisted or Config.Debug) then
            ...
            if GetVehiclePedIsUsing(playerPed) ~= 0 then
                ...
                if vehicleWhitelist[GetVehicleClass(vehicle)] then
                    ...
                    if Config.Timer['Speeding'] == 0 and speedlimit then
                    ...
                    if Config.Timer['Autotheft'] == 0 and (IsPedTryingToEnterALockedVehicle(playerPed) or IsPedJacking(playerPed) or (IsPedGettingIntoAVehicle(playerPed) and GetSeatPedIsTryingToEnter(playerPed) == -1 and not IsPedAPlayer(GetPedInVehicleSeat(vehicle, -1)))) then
                        local veh = vehicleData(vehicle)
